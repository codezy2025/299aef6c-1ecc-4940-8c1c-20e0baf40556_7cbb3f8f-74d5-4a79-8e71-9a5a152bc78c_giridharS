/**
 * 🏗️  DEVELOPMENT GUIDE - Database Integration & Vector Stores Service
 * 
 ...
 * - search(query: string): Promise<Database Integration & Vector Stores[]>
 * - bulkDelete(ids: string[]): Promise<void>
 * - export(): Promise<Blob>
 * - getStats(): Promise<{Database Integration & Vector StoresStats}>
 * 
 * 🔧 Error Handling:
...
import {
  DatabaseIntegrationVectorStore,
  DatabaseIntegrationVectorStoreCreate,
  DatabaseIntegrationVectorStoreUpdate
} from '../types/Database Integration & Vector StoresTypes';

const API_BASE_URL = '/api/database-integration-vector-stores';

const databaseIntegrationVectorStoresService = {
  getAll: async (): Promise<DatabaseIntegrationVectorStore[]> => {
    const response = await axios.get<DatabaseIntegrationVectorStore[]>(API_BASE_URL);
    return response.data;
  },

  create: async (data: DatabaseIntegrationVectorStoreCreate): Promise<DatabaseIntegrationVectorStore> => {
    const response = await axios.post<DatabaseIntegrationVectorStore>(API_BASE_URL, data);
    return response.data;
  },

  update: async (id: string, data: DatabaseIntegrationVectorStoreUpdate): Promise<DatabaseIntegrationVectorStore> => {
    const response = await axios.put<DatabaseIntegrationVectorStore>(`${API_BASE_URL}/${id}`, data);
    return response.data;
  },
...
export default databaseIntegrationVectorStoresService;