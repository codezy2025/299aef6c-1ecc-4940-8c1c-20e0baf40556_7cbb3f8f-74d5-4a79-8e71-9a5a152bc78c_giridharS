/**
 * 🏗️  DEVELOPMENT GUIDE - Recommender Form Component
 * 
 ...
 * - initialData?: Partial<Recommender> (for edit mode)
 * - onCancel?: () => void
 * - isLoading?: boolean
...
import { RecommenderFormValues, RecommenderFormProps } from '../types/RecommenderTypes';

const RecommenderForm: React.FC<RecommenderFormProps> = ({ onSubmit }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<RecommenderFormValues>();

  const onSubmitHandler = (data: RecommenderFormValues) => {
    onSubmit(data);
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitHandler)} className="space-y-6">
      <div>
        ...
        />
        {errors.user_id && (
          <p className="mt-1 text-sm text-red-500">{errors.user_id.message}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.project_id && (
          <p className="mt-1 text-sm text-red-500">{errors.project_id.message}</p>
        )}
      </div>

      <div>
        <label htmlFor="user_question" className="block text-sm font-medium text-gray-700">
        ...
        <textarea
          ...
          rows={3}
          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          placeholder="Enter your question to generate follow-up suggestions..."
        />
      </div>
