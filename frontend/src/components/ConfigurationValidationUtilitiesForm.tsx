/**
 * 🏗️  DEVELOPMENT GUIDE - Configuration & Validation Utilities Form Component
 * 
 ...
 * - initialData?: Partial<Configuration & Validation Utilities> (for edit mode)
 * - onCancel?: () => void
 * - isLoading?: boolean
...
import { ConfigurationValidationFormValues } from '../types/Configuration & Validation UtilitiesTypes';

interface ConfigurationValidationFormProps {
  onSubmit: (data: ConfigurationValidationFormValues) => void;
}

const ConfigurationValidationForm: React.FC<ConfigurationValidationFormProps> = ({ onSubmit }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<ConfigurationValidationFormValues>();

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        <label htmlFor="configPath" className="block text-sm font-medium text-gray-700">
          Configuration Path
        </label>
        <input
          id="configPath"
          type="text"
          {...register('configPath', {
            required: 'Configuration path is required',
            pattern: {
              value: /^[\w\-\/.]+$/,
        ...
        />
        {errors.configPath && (
          <p className="mt-1 text-sm text-red-500">{errors.configPath.message}</p>
        )}
      </div>
      ...
      <div>
        ...
        <input
          ...
          {...register('modelName', {
            ...
            minLength: {
              value: 1,
              message: 'Model name must be at least 1 character',
            },
            pattern: {
              value: /^[a-zA-Z0-9\-_]+$/,
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          placeholder="What are the top 5 customers in the US?"
        />
        {errors.inputText && (
          <p className="mt-1 text-sm text-red-500">{errors.inputText.message}</p>
        )}
      </div>
...
export default ConfigurationValidationForm;