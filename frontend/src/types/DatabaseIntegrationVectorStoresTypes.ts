/**
 * 🏗️  DEVELOPMENT GUIDE - Database Integration & Vector Stores Types
 * 
 ...
 * - Create utility types for API responses (ApiResponse<Database Integration & Vector Stores>)
 ...
 * 
 * 💡 Example Extensions:
 * - export enum Database Integration & Vector StoresStatus { ACTIVE = 'active', INACTIVE = 'inactive' }
 * - export type Database Integration & Vector StoresSearchParams = Pick<Database Integration & Vector Stores, 'name' | 'status'>
 * - export type Database Integration & Vector StoresUpdateData = Partial<Omit<Database Integration & Vector Stores, 'id' | 'createdAt'>>
 */

export interface DatabaseIntegrationAndVectorStores {
  addQuestionSql(question: string, sql: string, tag?: string): string;
  addDdl(ddl: string): string;
  addDocumentation(doc: string): string;
  getSimilarQuestionSql(question: string): string[];
  getRelatedDdl(question: string): string[];
  getRelatedDocumentation(question: string): string[];
  getTrainingData(): Record<string, any>[];
  removeTrainingData(id: string): boolean;
  getRelatedTrainingDataCached(question: string): TrainingData;
}

export interface DatabaseIntegrationAndVectorStoresFormData {
  question: string;
  sql: string;
  tag?: string;
  ddl?: string;
