/**
 * 🏗️  DEVELOPMENT GUIDE - Recommender Types
 * 
 ...
 * - Create utility types for API responses (ApiResponse<Recommender>)
 ...
 * 
 * 💡 Example Extensions:
 * - export enum RecommenderStatus { ACTIVE = 'active', INACTIVE = 'inactive' }
 * - export type RecommenderSearchParams = Pick<Recommender, 'name' | 'status'>
 * - export type RecommenderUpdateData = Partial<Omit<Recommender, 'id' | 'createdAt'>>
 */

export interface Recommender {
  loadDocuments(userId: string, projectId: string): void;
  generateQuestions(userId: string, projectId: string): string[];
  generateDatabaseQuestions(userId: string, projectId: string): string[];
  generateQuestionsFromUserQuery(userId: string, projectId: string, userQuestion: string): string[];
}
